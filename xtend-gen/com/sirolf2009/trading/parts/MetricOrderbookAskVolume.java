...
public class MetricOrderbookAskVolume extends AsyncMetric implements IExchangePart {
  @PostConstruct
  public void createPartControl(final Composite parent) {
    this.init(parent, "Ask volume per second");
    final Consumer<IOrderbook> _function = (IOrderbook it) -> {
      try {
        ...
        final Function1<ILimitOrder, Boolean> _function_1 = (ILimitOrder it_1) -> {
          ...
          double _minus = (_doubleValue_2 - mid);
          return Boolean.valueOf((_minus <= 25d));
        };
        final Function1<ILimitOrder, Double> _function_2 = (ILimitOrder it_1) -> {
          double _doubleValue_2 = it_1.getAmount().doubleValue();
          return Double.valueOf((-_doubleValue_2));
        };
        ...
        final Double asks = IterableExtensions.<Double>reduce(IterableExtensions.<ILimitOrder, Double>map(IterableExtensions.<ILimitOrder>filter(it.getAsks(), _function_1), _function_2), _function_3);
        if ((asks != null)) {
          this.set(asks);
        }
