    ...
    final Consumer<OrderBook> _function = (OrderBook it) -> {
      try {
        ...
        final Function1<LimitOrder, Double> _function_2 = (LimitOrder it_1) -> {
          return Double.valueOf(it_1.getOriginalAmount().negate().doubleValue());
        };
        ...
        final Double asks = IterableExtensions.<Double>reduce(IterableExtensions.<LimitOrder, Double>map(IterableExtensions.<LimitOrder>filter(it.getAsks(), _function_1), _function_2), _function_3);
        if ((asks != null)) {
          this.set(asks);
        }
