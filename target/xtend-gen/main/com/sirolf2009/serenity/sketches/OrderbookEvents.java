    ...
    final Consumer<IUpdate> _function = new Consumer<IUpdate>() {
      ...
      public void accept(final IUpdate it) {
        if ((it instanceof UpdateOpen)) {
          ...
          if (_greaterThan) {
            synchronized (OrderbookEvents.this.plot) {
              ...
              if (_tripleEquals) {
                ...
                OrderbookEvents.this.plot.addPoint(_minus_1, Double.valueOf(((UpdateOpen)it).getPrice()).floatValue());
                double _remainingSize_1 = ((UpdateOpen)it).getRemainingSize();
                double _multiply_1 = (_remainingSize_1 * 2000);
                OrderbookEvents.this.plot.setPointSizes(PApplet.append(bids.getPointSizes(), Double.valueOf(Math.log(_multiply_1)).floatValue()));
              }
            }
          }
        } else {
